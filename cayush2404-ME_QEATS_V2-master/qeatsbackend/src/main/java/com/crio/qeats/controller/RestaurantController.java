...
public class RestaurantController {
  ...
  public ResponseEntity<GetRestaurantsResponse> getRestaurants(
    ...
    if(getRestaurantsRequest.getLatitude()!=null && getRestaurantsRequest.getLongitude()!=null
      ...
      && getRestaurantsRequest.getLongitude()>=-180 && getRestaurantsRequest.getLongitude()<=180){
        ...
        //There was a bug spotted where the app was not able to parse some different characters(Non ASCII) in restaurant_name 
        //Here we would be replacing those with some known ASCII character so the parsing won't render any problem:
        if(getRestaurantsResponse != null && !getRestaurantsResponse.getRestaurants().isEmpty()){
            for (Restaurant restaurant : getRestaurantsResponse.getRestaurants()) {
              String modified_name = restaurant.getName().replaceAll("[Â©éí]", "%");
              restaurant.setName(modified_name);
            }    
  ...
  @GetMapping(MENU_API)
  public ResponseEntity<GetRestaurantMenuResponse> getRestaurantMenu(GetRestaurantMenuRequest getRestaurantMenuRequest){

    log.info("getRestaurantMenu called with {}", getRestaurantMenuRequest);

    GetRestaurantMenuResponse getRestaurantMenuResponse = restaurantService.findMenuByRestaurantId(getRestaurantMenuRequest);
    
    if(getRestaurantMenuResponse!=null){
      return ResponseEntity.ok().body(getRestaurantMenuResponse);
    }
    else{
      return ResponseEntity.badRequest().body(null);
    } 
  ...
  public ResponseEntity<String> stats() {
    ...
    return ResponseEntity.ok().body("Hello, Welcome to Qeats!!");
  }
